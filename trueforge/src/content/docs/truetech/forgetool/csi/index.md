...
The base forgetool setup, includes a single-node setup of Longhorn, which can be easily tweaked from single-node to multi-node setup.
...
Hence for most users starting-out we would advice starting-out using Longhorn and have included it as the default storage backend (CSI) when deploying forgetool.
...
- forgetool already includes snapshot controller by default
